...
> **Prosjektformål:** Utvikle et personvernfokusert system for sanntidsidentifisering av kjøretøy og bilskilt ved bruk av Edge AI og offentlige API-er, med mulighet for videre skalering.
...
1. **AI / Image Processor:** Egenutviklet Python-container med egentrent AI modell - (YOLOv8m + EasyOCR) som leser RTSP-strøm, vasker bilder og trekker ut skiltnummer.
2. **MQTT (Ikke i bruk):** Broker for rask meldingsutveksling og IoT-kommunikasjon. **FORANDRINGER -> Leser av mediafil/sti istedenfor**!
...
Dette er et API som er offentliggjort ved bestilling som privatperson. API-et gir tilgang til kjøretøyopplysninger og teknisk data, men ikke eieropplysninger. Når AI-modellen detekterer et skiltnummer, utføres et teknisk enkeltoppslag for å berike dataene i dashbordet med:
* Kjøretøyets merke og modell.
* Førstegangsregistrert i Norge.
* Status for EU-kontroll i form av tid samt om det er godkjent.

### API-integrasjon i historikk
Historikk siden blir også beriket med data ved klikk på enkelte biler utvalgt:
* Vekt.
* Mål -> Lengde på bil
* Inneholder også dataene fra dashboardet.
...
* **[08 - Skalering og Cloud-Trening: Fra Prototype til Produksjon](./Docs/08-Videre-Dyp-Trening.md):** Detaljer rundt hvordan datasettet ble oppskalert til 22 000 bilder med Gemini 2.0 Flash som annoterte bildene, og den endelige sky-treningen (`best.pt`) på en RTX 4090 via RunPod.
...
* **[11 - Script endringer: Fra en `Flask server` til `RTSP-Stream`](/Docs/11-Sript-changes-and-tailscale.md):** Detaljer om nettverksprotokollene som ble endret og etter kritisk tenkning samt analyse, gikk valget over til Tailscale.
...
* **[14 - Cloudflare Zero Trust og Sikkerhet](/Docs/14-Cloudflare-Zero-Trust.md):** Detaljer om Cloudflare, Policies og konfigurering av sikkerhet for applikasjonen.
* **[15 - Hardware-krise og løsning: Fra Global Shutter til Pixel 10 Pro](/Docs/15-Hardware-løsning.md):** Detaljer om hardware problemer som ble støtt på, og hvordan dette ble løst rett før demonstrasjonen på USNExpo - teknologimesse.
